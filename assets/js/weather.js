...
const todayRender = () => {
   ...
   const realFeelTemperature = Math.round(
      forecast.RealFeelTemperature.Metric.Value
   )
...
const weekendRender = () => {
   ...
   const weekForecastDetail = forecasts
      .map((forecast, index) => {
         ...
         const dateString =
            date.getTime() === today.getTime()
               ? '오늘'
               : date.getMonth() + 1 + '월 ' + date.getDate() + '일' // 월과 일을 문자열로 만듦
